...
samples <- extract.samples(m.3.power)
  ...
  m.3.power2 <- map2stan(m.3.power, data=df, iter=6000, warmup=1000, chains=3, cores = 4)
...
#saving 
save(df_HPDI_accuracy, file = "df_HPDI_accuracy3.RData")
...
for(runs in 1:200) {
  ...
  #saving 
  save(df_HPDI_tiles, file = "df_HPDI_tiles.RData")
...
df_HPDI_mathrate <- data.frame(c_low = rep(0, 200), c_high = rep(0, 200))
...
for(runs in 1:200) {
  ...
  df_HPDI_mathrate$c_low[runs] <- HPDI(samples$bC, prob = 0.95)[1]
  df_HPDI_mathrate$c_high[runs] <- HPDI(samples$bC, prob = 0.95)[2]
  
  #save 
  save(df_HPDI_mathrate, file = "df_HPDI_mathrate.RData")
